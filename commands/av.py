...
async def avatar(ctx, user: discord.Member):
	embed = discord.Embed(description="{.mention}".format(user), color=int(random.choice(colorsEmbed)))
...
async def avatar_error(ctx, error):
	if isinstance(error, commands.MissingRequiredArgument):
		embed = discord.Embed(description="{.mention}".format(ctx.message.author), color=int(random.choice(colorsEmbed)))
