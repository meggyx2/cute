...
async def editsnipe(ctx):
	try:
		to_slice = editsnipe_msgs[str(ctx.message.channel.id)]
	...
	try:
		...
		return
	embed = discord.Embed(description="{}".format(sniping[0]), color=0xFFFFFF, timestamp=editsnipe_msgs_time[str(ctx.message.channel.id)])
