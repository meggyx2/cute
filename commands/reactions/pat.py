...
async def pat(ctx, user: discord.Member):
	embed = discord.Embed(description="**{.message.author.name}** pats **{.name}**. <:iloveyou:656842699344707604>".format(ctx, user), color=0xFFFFFF, timestamp=datetime.utcnow())
...
async def pat_error(ctx, error):
	if isinstance(error, commands.MissingRequiredArgument):
		embed = discord.Embed(description="**babi** pats **{.message.author.name}**. <:iloveyou:656842699344707604>".format(ctx), color=0xFFFFFF, timestamp=datetime.utcnow())
		embed.set_image(url=random.choice(pat_gifs))
		await ctx.send(embed=embed)
	elif isinstance(error, commands.BadArgument):
		embed = discord.Embed(description="**babi** pats **{.message.author.name}**. <:iloveyou:656842699344707604>".format(ctx), color=0xFFFFFF, timestamp=datetime.utcnow())
		embed.set_image(url=random.choice(pat_gifs))
		await ctx.send(f"**{ctx.message.author.name}** member not found, I patted you instead", embed=embed)
	else:
