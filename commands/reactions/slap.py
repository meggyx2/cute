...
async def slap(ctx, user: discord.Member):
	embed = discord.Embed(description="**{.message.author.name}** slaps **{.name}**. <:pensive_heart:681955741279715349>".format(ctx, user), color=0xFFFFFF, timestamp=datetime.utcnow())
...
async def slap_error(ctx, error):
	if isinstance(error, commands.MissingRequiredArgument):
		embed = discord.Embed(description="**babi** slaps **{.message.author.name}**. <:pensive_heart:681955741279715349>".format(ctx), color=0xFFFFFF, timestamp=datetime.utcnow())
		embed.set_image(url=random.choice(slap_gifs))
		await ctx.send(embed=embed)
	elif isinstance(error, commands.BadArgument):
		embed = discord.Embed(description="**babi** slaps **{.message.author.name}**. <:pensive_heart:681955741279715349>".format(ctx), color=0xFFFFFF, timestamp=datetime.utcnow())
		embed.set_image(url=random.choice(slap_gifs))
		await ctx.send(f"**{ctx.message.author.name}** member not found, I slapped you instead", embed=embed)
	else:
